(function(Spine, $)
{
	...
	{	
		elements: {
			'#top_settings': 'top_element',
			'#menu_settings': 'menu_element',
			'#inner_settings': 'inner_menu_element',
			'.title': 'title_element',
			'.title .text': 'title_text_element'
		},
		
		events: {
			'tap #menu_settings a': 'setCurrent',
			'swipe .switch': 'emulateChange',
			'tap .go_back': 'returnMenu',
			'change #inner_settings input': 'optionChanged',
			'change #inner_settings textarea': 'optionChanged',
			'change #inner_settings select': 'optionChanged'
		},
		...
		init: function()
		{
			...
			this.options_toggle = $('#options_toggle');
			this.go_back = this.el.find('.go_back');
			
			this.open_content_class = 'open_settings';
			...
			Spine.bind('toggleSettings', this.proxy(this.toggleSettings));
			
			if(!localStorage.getItem('use_compact'))
				localStorage.setItem('use_compact', 'false');
		},
		
		useWebUiSettings: function()
		{
			if(this.settings['webui._cookie.updateInterval'])
				Spine.trigger('setIntervals', this.settings['webui._cookie.updateInterval'].value);
			
			var use_compact = localStorage.getItem('use_compact');
			$('#torrents').toggleClass('compact', use_compact && use_compact !== 'false');
		},
		...
		optionChanged: function(e)
		{
			...
			if(me.is(':checkbox'))
			{
				...
				this.el.find('[data-depends="' + me.attr('id') + '"]').prop('disabled', !value);
		...
		setSettings: function(s, v)
		{
			if(!_.isArray(s))
			{
				client.raptor.api.post({ action: 'setsetting', s: s, v: v });
			}else{
				var params = '';
				...
				client.raptor.api.post('action=setsetting&' + params);
			}
		},
		
		returnMenu: function()
		{
			this.el.find('.settings_content').removeClass('done');
			this.el.removeClass('sub');
			this.title_text_element.text(this.title_element.data('title'));
		...
		startTemplates: function()
		{
			...
			{
				this.templates = {
					section: Handlebars.compile($('#tmpl_options_section').html())
				};
			}
		},
		
		toggleSettings: function(force_active)
		{
			if(force_active === undefined && !this.active)
				Spine.trigger('closeModals');
			
			this.activate(force_active);
			this.all_content.toggleClass(this.open_content_class, this.active);	
			this.returnMenu();
		},
		
		emulateChange: function(e)
		{
		...
		setCurrent: function(e)
		{
			...
			{
				html += self.templates.section(item);
			});
			
			this.el.addClass('sub');
			
			this.inner_menu_element.html(html);
			
			this.title_text_element.text(me.text());
		...
		{
			var self = this;
			$('select, input, textarea', this.inner_menu_element).each(function()
			{
				var el = $(this);
		...
		activate: function(force_active)
		{
			this.active = force_active === undefined ? !this.all_content.hasClass(this.open_content_class) : force_active;
			this.options_toggle.toggleClass('open', this.active);
			if(this.active)
				this.loadAll();
		},
		...
		loadAll: function()
		{
            ...
            client.raptor.api.post(
            	...
            	function(data) 
				{
					_.each(data.settings, function(s)
					{
						self.settings[s[0]] = {
							type: s[1],
							value: s[2],
						...
						if(s[0] == 'webui.cookie')
						{
							var parsed = JSON.parse(s[2]);
							_.each(parsed, function(val, key)
							{
								self.settings['webui._cookie.' + key] = {
